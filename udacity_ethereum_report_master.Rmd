...
for Data Analysis project. This is a perfect opportunity to explore data collected 
related to the cryptoassset Ether and it's blockchain of Ethereum. 
...
setwd('/Users/RichardJamesLopez/Dropbox/udacity_ethereum_data')
...
```{r echo=FALSE, Description_of_Variables}
str(all_data)
```
...
> * <b>Market_Cap</b> - Value of ether multiplied by the supply of Ether coins at the time of the valualation. 


### Block_Size Univariate Charts
...
```{r echo=FALSE, Summary_of_Block_Size}
summary(all_data$Block_Size)
```

### Block_Size Histogram V1
...
qplot(data = all_data, x = Block_Size, 
      ...
      color= I('pink'), 
      fill = I('#099DD9')) +
  scale_x_continuous(limits = c(min(all_data$Block_Size),max(all_data$Block_Size)))
```

### Block_Size Histogram V2
...
```{r echo=FALSE, Univariate_Plots_V4}
...
q2 <- qplot(data = all_data, x = Transactions, 
      ...
      color= I('green'), 
      fill = I('#4DBD33'))+
  scale_x_continuous(limits = c(min(all_data$Transactions),max(all_data$Transactions)))
...
```{r echo=FALSE, message=FALSE, warning=TRUE, Univariate_Plots_V3}
qplot(data = all_data, x = Block_Size, 
      binwidth = 250,
      color= I('pink'), 
      fill = I('#099DD9')) +
  scale_x_continuous(limits = c(quantile(all_data$Block_Size,0.05),quantile(all_data$Block_Size,0.95)))
```
...
all_data_subset = all_data[, c('Unix_Time_Stamp', 'Transactions', 'Block_Size', 'Price_Change', 'Supply', 'Hashrate')]
...
s2 <-ggplot(data = all_data_block_size_small, aes(x = Hashrate, y = Block_Size)) +
  geom_jitter(color = I('#099DD9')) +
...
ggplot(data = all_data_hashrate_small, aes(x = Hashrate, y = Block_Size)) +
  geom_jitter(color = I('#099DD9')) +
...
ggplot(data = all_data, aes(x = Market_Cap, y = Transactions, color= I('#099DD9'))) +
  geom_jitter() + 
  geom_smooth()
...
ggplot(data = all_data, aes(x = Unix_Time_Stamp, y = Price), color= I('#099DD9')) +
  geom_line()
...
ggplot(data = all_data, aes(x = Unix_Time_Stamp, y = log10(Price + 1), color= I('#099DD9'))) +
  geom_line()

```

### Price_Change <> Transactions
...
ggplot(data = all_data_post_zero, aes(x = Price_Change, y = Transactions), color= I('#099DD9')) +
  geom_jitter(color= I('#099DD9')) + 
  geom_smooth()
...
```{r echo=FALSE, message=FALSE, warning=TRUE, Bivariate_Plots_V4_Trimmed}
ggplot(data = all_data_post_zero, aes(x = Price_Change, y = Transactions), color= I('#099DD9')) +
  geom_jitter(color= I('#099DD9')) +
  xlim(quantile(all_data$Price_Change,0.05),quantile(all_data$Price_Change,0.95)) +
  geom_smooth()
...
p3 <- ggplot(data = all_data_Transactions_1st, aes(x = Price_Change, y = Transactions)) +
  geom_jitter(color= I('#099DD9')) + 
  geom_smooth() +
  ggtitle("1st Half of Time Series", subtitle = "with Price_Change <> # of Transactions")

p4 <- ggplot(data = all_data_Transactions_2nd, aes(x = Price_Change, y = Transactions)) +
  geom_jitter(color= I('#9B30FF')) + 
  geom_smooth() +
  ggtitle("2nd Half of Time Series", subtitle = "with Price_Change <> # of Transactions")
...
ggplot(data = all_data, aes(x = Block_Size, y = Transactions, color = I('#099DD9'))) +
  geom_jitter(alpha = 1/5) +
  facet_wrap(~ Day, ncol = 7) +
  scale_x_continuous(limits = c(0, max(all_data$Block_Size)), breaks = c(0,max(all_data$Block_Size),0))
...
p5 <- ggplot(all_data_Transactions_2nd, aes(x = Block_Difficulty, y = Block_Size, colour = Price_Change)) +
  geom_point(alpha = 1) +
  scale_color_gradient(guide = "colourbar", low = "white", high = "#336600")

p6 <- ggplot(all_data_Transactions_2nd, aes(x = Block_Difficulty, y = Block_Size, colour = Price_Change)) +
  geom_point(alpha = 1) +
  scale_color_gradient(guide = "colourbar", low = "#990033", high = "white")
...
p7 <- ggplot(all_data_Transactions_1st, aes(x = Block_Difficulty, y = Block_Size, colour = Price_Change)) +
  geom_point(alpha = 1) +
  scale_color_gradient(guide = "colourbar", low = "white", high = "#336600")


p8 <- ggplot(all_data_Transactions_1st, aes(x = Block_Difficulty, y = Block_Size, colour = Price_Change)) +
  geom_point(alpha = 1) +
  scale_color_gradient(guide = "colourbar", low = "#990033", high = "white")
...
### Plot One - Price_Change <> Transactions
```{r echo=FALSE, message=FALSE, warning=TRUE, Bivariate_Plots_V4_Final}
all_data_post_zero <- subset(all_data, Unix_Time_Stamp >= 1439251200)

p1 <- ggplot(data = all_data_post_zero, aes(x = Price_Change, y = Transactions), color= I('#099DD9')) +
  geom_jitter(color= I('#099DD9')) + 
  geom_smooth()

p2 <- ggplot(data = all_data_post_zero, aes(x = Price_Change, y = Transactions), color= I('#099DD9')) +
  geom_jitter(color= I('#099DD9')) +
  xlim(quantile(all_data$Price_Change,0.05),quantile(all_data$Price_Change,0.95)) +
  geom_smooth()
...
### Plot Three - Price_Change <> Block Factors
...
```{r echo=FALSE, Univariate_Plots_Other_1}
